...
class AutoRetrainer:
    ...
    def create_training_dataset(self):
        ...
        X = pd.DataFrame([entry['data'] for entry in self.prediction_data], columns=feature_names)
        y = pd.Series([entry['prediction'] for entry in self.prediction_data], name='Machine failure')
    ...
    def get_retraining_status(self):
        ...
        status = {
            ...
            'last_retrain': self.metrics_data[-1]['timestamp'] if self.metrics_data else 'Never',
            'last_accuracy': self.metrics_data[-1]['accuracy'] if self.metrics_data else 0
        }
